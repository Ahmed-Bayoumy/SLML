...
class DataSet(pydantic.BaseModel):
		...
		def __multiply__(self, other):
			if isinstance(other, DataSet):
				...
				raise ExceptionError(name="DataSet::multiply::", message="couldn't recognize the multiplier type!") 
		...
		def tril_solve(self, L: DataSet, b: DataSet):
			n = L.ncols
			if n != L.ncols:
							ExceptionError(name="Dataset::add_rows::", message="bad dimensions")	
...
class Surrogate(metric_t(), surrogate_utils()):
		...
		def predict (self, XX:DataSet, ZZ: DataSet, std:DataSet, ei:DataSet, cdf:DataSet):
				""" /*---------------------------------------------------------------------*/
						/*               predict                                               */
				...
				if XX.ncols != self._n:
						raise ExceptionError(message="predict::", message="dimension error")
				
				# Create the  (non-scaled) output matrix
				ZZ = DataSet(name="ZZ", nrows=XX.nrows, ncols=self._m)

				#  Scale the input (XX----> XXs)
				XXs: DataSet = self._trainingSet.X_scale_DataSet(XX)


				# Init the expected improvement
				if ei:
						ei.points = [[-np.inf] * ei.ncols] * ei.nrows
				...
				self.predict_private(XX, ZZ, std, ei, cdf)
				pxx: int = XX.nrows
				if ZZ is not None:
					for j in range(self._m):
						if self._trainingSet.get_Z_nbdiff(j)==1:
							for i in range(ppx):
								ZZ.points_set(i,j,0.)
				
				ZZ.replace_nan(np.inf)
				std.replace_nan(np.inf)
				ei.replace_nan(-np.inf)
				cdf.replace_nan(0)
				...
				if ZZ is not None:
					ZZ.name = "ZZ"
					self._trainingSet.ZE_unscale(ZZ)
							
				if std is not None:
					std.name = "std"
					self._trainingSet.ZE_unscale(std)
							
				if ei is not None:
					ei.name = "ei"
					self._trainingSet.ZE_unscale(ei)
		...
		def predict_private(self, XXs: DataSet, ZZs: DataSet, std: DataSet, ei: DataSet, cdf:DataSet):
			...
			# TODO: implement check_ready __file__, __Function__ and __line__
			self.check_ready()
			pxx: int = XXs.nrows
			# TODO: implement _trainingSet.get_fs_min
			fs_min: float = self._trainingSet.get_fs_min()
			if ZZs is not None or ei is not None or cdf is not None:
				# TODO: implement predict_private_XXZZ 
				self.predict_private_XXZZ()
											
			# Prediction of statistical data
			if std is not None or ei is not None or cdf is not None:
				if std is not None: std.points = [[-np.inf] * std.ncols]	* std.nrows
				else: std = DataSet(name="std", nrows= pxx, ncols=self._m)
				if cdf is not None: cdf.points = [[-np.inf]* cdf.ncols]	* cdf.nrows
				if ei is not None: ei.points = [[-np.inf]* ei.ncols]	* ei.nrows
			
			# Use normalized distance to closest and rmse as std
			dtc: DataSet = self._trainingSet.get_distance_to_closest(XXs=XXs)
			dtc.name = "dtc"

			for j in range(self._m):
				# Set std (use a proxy)
				s: float = self.get_metric(mt=self.METRIC_RMSE)	
				std.__set_col__(dtc+s, j)
				
				if self._trainingSet.get_bbo(j) == bbo_t.BBO_OBJ:
					# Compute CDF
					if cdf is not None:
						for i in range(pxx):
							cdf.points_set(i,j, self.normcdf(ZZs.points_get(i,j), std.points_get(i,j)))
					if ei is not None:
						for i in range(pxx):
								ei.points_set(i,j, self.normei(ZZs.points_get(i,j), fs_min))
				elif self._trainingSet.get_bbo(j) == bbo_t.BBO_CON:
					if cdf is not None:
						#           // Scaled Feasibility Threshold
						cs: float = self._trainingSet.Z_scale(0., j)
						for i in range(pxx):
							cdf.points_set(i,j, self.normcdf(ZZs.points_get(i,j), std.points_get(i,j)))

		def predict(self, XX: DataSet, ZZ: DataSet):
			self.check_ready()

			# Check the number of columns in XX
			if XX.ncols != self._n:
				raise ExceptionError(name="predict::", message="dimension error")			
			ZZ: DataSet = DataSet(name="ZZ", nrows=XX.nrows, ncols = self._m)
			# // Scale the input
			XXs: DataSet
			XXs = copy.deepcopy(XX)
			self._trainingSet.X_scale_DataSet(XXs)
			
			# Call the private prediction with normalized input XXs
			self.predict_private_XXZZ(XXs, ZZ)
			self._trainingSet.Z_unscale_DataSet(ZZ)
...
class PRS(Surrogate):
		...
		def compute_design_matrix(self, Monomes:DataSet, Xs: DataSet):
				...
				for k in range(nbMonomes):
						...
						for j in range(n):
							if self._trainingSet.get_X_nbdiff(j)>1:
								exponent = int(Monomes.points_get(k, jj))
								if exponent>0:
									for i in range(p):
										v = h.points_get(i, 0)
										v *= Xs.points_get(i, jj) ** exponent
										h.points_set(i, 0, v)
								jj += 1
						H.__set_col__(h, k)
				return H
		
		def compute_alpha(self)->bool:
			""" Compute Alpha """
			Ht: self._H.__transpose__()
			Zs: DataSet = self.get_matrix_Zs()
			
			# Ridge'
			r: float = self._param.ridge
			# Compute COEFS
			if r>0:
				self._Ai = np.multiply(Ht, self._H)+r* DataSet(np.eye(self._q)).__cholesky_inverse__()
			else:
				self._Ai = (Ht*self._H).__cholesky_inverse__()
			
			self._alpha: DataSet = self._Ai * (Ht * Zs)
			self._alpha.name = "alpha"

			if np.isnan(self._alpha.points):
				return False
			return True
		
		def predict_private(self, XXs: DataSet)->DataSet:
			self.check_ready()
			return self.compute_design_matrix(self._M, XXs) * self._alpha
		
		def get_matrix_Zvs(self):
			self.check_ready()
			if self._Zvs is not None:
				self._Zvs: DataSet
				# // Projection matrix
				Zs = self.get_matrix_Zs()
				dPiPZs: DataSet = self.get_matrix_dPiPZs(self._Ai, self._H, self.Zs)

				# // dPi is the inverse of the diag of P 
				# // Compute _Zv = Zs - dPi*P*Zs
				self._Zvs = Zs - dPiPZs
				self._Zvs.__replaceNAN__(np.inf)
				self._Zvs.name = "Zvs"
			return self._Zvs
		
		def get_nb_PRS_monomes(nvar: int, degree: int):
			""" /*-----------------------------------------*/
		...
		def get_PRS_monomes(nvar: int, degree: int)->DataSet:
			z: List[float] = [0.]*nvar
			M: DataSet = DataSet(name="Monomes", nrows=1, ncols=nvar)
			continuer: bool
			...
			for c in range(1,min(degree,nvar)+1):
				
				for d in range(c, degree+1):
					# // First monome (c,d)
					z[0] = d-c+1
					for i in c:
						z[i] = 1.
					for i in range(c, nvar):
						z[i] = 0.	
					continuer = True
					while continuer:
						M.add_rows(z)
