# ------------------------------------------------------------------------------------#
#  Simple Surrogate Models Library - SML                                              #
#                                                                                     #
...
class Kriging(surrogateModel):
  ...
  def __init__(self, type: str, x: np.ndarray = None,
               y: np.ndarray = None, xfile='x_train.dat', yfile='y_train.dat',
               options={}, model_db='model.db', scf_func='standard'):
    self.options = options
...
class bmSM:
  ...
  def RB_RBF(self):
    ...
    model = {'sm': self.sm}
    data = {'xt': xt, 'yt': yt}
    datap = {'xp': self.sm.x.points, 'yp': self.sm.yp.points}
    path = os.getcwd()
    testPath = os.path.abspath(f'{path}/tests/training_data')
    filepath = os.path.join(testPath, 'RBF_RB_training.pkl')
    filepathp = os.path.join(testPath, 'RBF_RB_predicted.pkl')
    filepathm = os.path.join(testPath, 'RBF_RB_model.pkl')
    with open(filepath, 'wb') as pck:
      pickle.dump(data, pck)

    with open(filepathp, 'wb') as pck:
      pickle.dump(datap, pck)

    with open(filepathm, 'wb') as pck:
  ...
  def RB_min_true(self):
    self.nsqp = 0
    return minimize(rosen, self.xhist, callback=self.callbackF, method="SLSQP", jac=rosen_der, tol=1e-6, bounds=np.array([[0.8, 1.2], [0.8, 1.2]]))
  ...
  def RB_min_krig(self):
    self.nsqp = 0
    return minimize(self.rosen_krig, self.xhist, callback=self.callbackF, method="SLSQP", tol=1e-6, bounds=np.array([[.5, 1.5], [.5, 1.5]]))

  def RB_min_rbf(self):
    self.nsqp = 0
    return minimize(self.rosen_rbf, self.xhist, callback=self.callbackF, method="SLSQP", tol=1e-6, bounds=np.array([[.5, 1.5], [.5, 1.5]]))

  def RB_kriging(self):
    v = np.array([[-2.0, 2.0], [-2.0, 2.0]])
    n = 50
    # sampling = FullFactorial(ns=n, vlim=v, w=None, c=False)
    sampling = halton(ns=n, vlim=v)
    xt = sampling.generate_samples()
    yt = self.RB(xt, None)
    opts: Dict = {}
